...
#define Nexp 7 //numero de experimentos, posteriormente se hace una media
#define Kp 167.4145 //Valor de Kp para coef amortiguamiento 0,3
...
//interrupcion timer
void moving (){
  double error= Referencia - double(pulses)*2*PI/REVOLUCIONES;//todo calculado en radianes
  double realimentacion= error*Kp;
  if(realimentacion > 12) {//valor de saturacion max
     realimentacion = 12;
     setVoltage(realimentacion);
  }else if (realimentacion < -12) {//valor de saturacion max en sentido negativo
     realimentacion = -12;
     setVoltage(realimentacion);
  }
  else
...
//metodo que imprime por el serial monitor la mediade todos los experimentos realizados
void print() {
